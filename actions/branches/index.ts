...
import { CreateBranchPayload, CreateBranchResponse, GetBranchesResponse, GetBranchResponse, UpdateBranchPayload, UpdateBranchResponse } from "@/lib/http-service/branches/types";
...
export async function deleteBranchAction(branchId: string): Promise<APIResponse<void>> {
    return await branchesService.deleteBranch(branchId);
}
...
export async function getBranchesAction(): Promise<APIResponse<GetBranchesResponse>> {    
    return await branchesService.get_branches();
}
